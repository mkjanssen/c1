...
%\usecolortheme{owl}
\setbeamercolor{progress bar}{fg=gold,bg=gold!30}
...
\title{\S 3.3: Global Optimization}
%\subtitle{Welcome!}
...
%	\pause
%	
%	Upshot: When limiting ourselves to a finite-length interval, we consider the function's behavior at the endpoints. We also often use the phrase \alert{absolute extrema} instead of \alert{global extrema}.
%
%}


\frame{
	\frametitle{Activity 3.3.2}
...
\alert{Big Huge Important Fact:} The only places these extreme points can occur are the critical numbers or the endpoints $a$ and $b$.

}

\frame{
	\frametitle{Activity 3.3.3}

}

\frame{
	\frametitle{Activity 3.3.4: Toward Applications}
	
	Math is fun and cool and all that, but we can also use math to solve problems from other disciplines! Huh. Who knew?
	
	Anyway: if a function represents a (possibly) changing quantity, we can use calculus to optimize! That is, we can find maxima and minima. 
	
	\pause
	
	Now work on \alert{Activity 3.3.4}.
