...
\title{\S 2.8: Using Derivatives to Evaluate Limits}
%\subtitle{Welcome!}
...
\frame{
	...
	Observe: the limit definition of the derivative yields an \alert{indeterminate form}, $\frac{0}{0}$.\pause\ More generally, limits like
	
	\[
		\lim\limits_{x\to 0}  \frac{\sin x}{x}
	\]
	also have the form $\frac{0}{0}$.

}


\frame{
	\frametitle{Guillaume de L'H\^{o}pital}
	
	\begin{minipage}[t]{0.45\linewidth}
	
\begin{itemize}
	\item 1661-1704
	\item First calculus textbook: \emph{Infinitesimal calculus with applications to curved lines}
\end{itemize}
\vfill
\end{minipage}
\hspace{0.5cm}
...
\includegraphics[width=\textwidth]{img/lopital.jpg}
\end{minipage}
...
\frame{
	...
	Idea: replace the function with its local linearization!
	\pause
	
	\begin{theorem}[L'H\^{o}pital's Rule I]
		Let $f$ and $g$ be differentiable at $a$, with $f(a) = g(a) = 0$ and $g'(a)\ne 0$. Then
		\[
	...
	\alert{Warning:} you need to make sure the hypotheses of the theorem are met before applying it!

}


\frame{
	\frametitle{Activities 2.8.2-2.8.3}
...
\frame{
	\frametitle{Limits involving $\pm \infty$}
	
	
	Consider $f(x) = \frac{1}{x^2}$; what is happening as $x\to 0^+$?
...
\frame{
	\frametitle{What we mean}
	
	What do we mean by the statements
	\[
		\lim\limits_{x\to 0^+} \frac{1}{x^2}= \infty,  \ \lim\limits_{x\to \infty } \frac{1}{x^2}=0, \ \text{ and } \lim\limits_{x\to \infty} e^x = \infty?
	\] \rem{end behavior}

}

\frame{
	\frametitle{L'H\^{o}pital: To Infinity, and Beyond}
	
	What should we do with $\lim\limits_{x\to \infty} \frac{x^3+1}{e^x}$? There's no ``local linearization'' at infinity.
	
	\pause
	
	\begin{theorem}[L'H\^{o}pital's Rule II]
		If $f$ and $g$ are differentiable and both approach 0 or both approach $\pm \infty$ as $x\to a$ (where $-\infty \le a \le \infty$), then 
		\[
			\lim\limits_{x\to a} \frac{f(x)}{g(x)} = \lim\limits_{x\to a} \frac{f'(x)}{g'(x)},
		\]
...
\frame{
	...
	Suppose $\frac{f(x)}{g(x)}$ has the form $\infty/\infty$ as $x\to\infty$. What is meant by:
	
	\begin{itemize}
	\item $\lim\limits_{x\to\infty} \frac{f(x)}{g(x)} = 0$?\pause
	\item $\lim\limits_{x\to \infty} \frac{f(x)}{g(x)} = \infty$?\pause
	\item $\lim\limits_{x\to \infty} \frac{f(x)}{g(x)} = c$, where $c$ is some finite number?
\end{itemize}
