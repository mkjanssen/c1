...
\title{\S 5.2: The Second Fundamental Theorem}
%\subtitle{Welcome!}
\author{Dr.\ Mike Janssen}
\date{April 16, 2021}%, 2020}
...
\frame{
	...
	so:\pause
	\begin{enumerate}
	\item ($\rightarrow$) If we have a graph of $f$ and we can compute the area bounded by $f$ on $[a,b]$, we can compute the change in an antiderivative $F$ over the interval
	\item ($\leftarrow$) If we can find an algebraic formula for $F$, we can compute the net signed area bounded by $f$ \rem{erf}
\end{enumerate}
...
\frame{
	\frametitle{From Integral Functions to FTC II}
...
\frame{
	\frametitle{Activity 5.2.2}

}


\frame{
	\frametitle{Understanding Integral Functions}
	
	Suppose 
	\[
		F(x) = \int_c^x f(t)\, dt,
	\]
	where $F(c) = 0$.\pause\ Then:
	\[
		\frac{d}{dx} [F(x)] = \frac{d}{dx}\left[\int_c^x f(t)\, dt\right] = f(x).
	\]
	\pause
	
	Thus, we can understand the graph of $F$ via $f$, even if an algebraic formula for $F$ eludes us.
...
\frame{
	\frametitle{Integration: The Big Picture}
	
	\begin{itemize}
	\item Integrals give us net signed area of the functions we integrate over an interval
	\item \alert{FTC I}: We can calculate exact values of the integral using any antiderivative
	\item \alert{FTC II}: the integral function gives a unique antiderivative
	\item \alert{FTC consequence}: integration and differentiation are \alert{almost} inverse processes
\end{itemize}
