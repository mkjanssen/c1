...
\title{\S 5.1: Constructing Accurate Graphs of Antiderivatives}
%\subtitle{Welcome!}
...
\frame{
	...
	\begin{itemize}
	\item CoL 3: November 23 (Monday before Thanksgiving)
	\item CoL 3 interviews: done over Zoom the last week of class
...
\frame{
	\frametitle{A Subtheme of the course}
	
	\alert{Question}: Given the derivative of an unknown function $f$, how much information can we obtain about $f$ itself?
...
\frame{
	...
	If $F$ and $G$ are antiderivatives of the same function, then $F-G$ must be a constant.
	
	\pause
	
	\alert{Example:} Consider $f(x) = 2x$. 

}

\frame{
	\frametitle{Activity 5.1.3}
...
\frame{
	...
	We can carry this further for continuous functions $f$:
	
	\[
		A(x) = \int_a^x f(t)\, dt
	\]
	is known as an \alert{integral function}.\pause\ \alert{What information does it give us?}

}

\frame{
	\frametitle{Activity 5.1.4}
