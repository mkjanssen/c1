...
\title{\S 2.1: Elementary Derivative Rules}
%\subtitle{Welcome!}
...
\frame{
	\frametitle{Constants, Powers, and Exponentials}
	
	
	
	\begin{itemize}
	\item $ \frac{d}{dx} [c] = 0$\pause
	
	\item $ \frac{d}{dx} [x^m] = m x^{m-1}$\pause
	
	\item $ \frac{d}{dx} [ m^x ] \ln(m) m^x$
\end{itemize}
...
\frame{
	\frametitle{Constant Multiples and Sums}
	
	\begin{itemize}
	\item $ \frac{d}{dx} [c f(x)] = c f'(x)$\pause
	
	\item $ \frac{d}{dx} [g(x) + h(x)]= g'(x) + h'(x)$
\end{itemize}

}

\frame{
	\frametitle{Activity 2.1.3}
...
\frame{
	...
	\begin{itemize}
	\item The derivative of a polynomial is a polynomial\pause
	
	\item The derivative of an exponential is an exponential\pause
	
	\item We must not lose sight of what the derivative \emph{means}--it still describes the instantaneous rate of change of the function being differentiated
\end{itemize}
