...
\title{\S 2.3: Product and Quotient Rules}
%\subtitle{Welcome!}
...
\frame{
	...
	Understand how the differentiation process interacts with our other algebraic operations: multiplication and division.
...
\frame{
	...
	\begin{theorem}
If $f$ and $g$ are differentiable functions of $x$, then their product $P(x) = f(x)\cdot g(x)$ is also a differentiable function, and
\[
P'(x) =	f(x) g'(x) + g(x) f'(x).
\]
\end{theorem}
...
\frame{
	...
	Let's calculate the derivative of $s(x) = \sin x \cos x$.

}

\frame{
	\frametitle{Activity 2.3.2}

}

\frame{
	\frametitle{The idea of the quotient rule}
	
	How do we differentiate $Q = f/g$? By applying the product rule to $f = g Q$.
...
\frame{
	\frametitle{Activity 2.3.3}

}

\frame{
	\frametitle{Activity 2.3.4}
	
	%Scavenger Hunt? Or Activity 2.3.4.
