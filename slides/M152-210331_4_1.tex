...
\title{\S 4.1: Determining Distance Traveled from Velocity}
%\subtitle{Welcome!}
...
\frame{
	...
	The area under our (positive) velocity function $v(t)$ from $t=a$ to $t=b$ gives the distance the object travels from $t=a$ to $t=b$.

}


\frame{
	\frametitle{Activity 4.1.2}
	

	
	\begin{center}
		\includegraphics[width=3in]{img/4_1_Act1-eps-converted-to.pdf}
	\end{center}
...
\frame{
	...
	When the velocity is not constant, we can estimate it over (small) intervals by assuming it is!
	
	\pause
	
	We estimate the area under a curve using rectangles.
	
	\pause
	
	Alternative approach (at least in certain circumstances): find an \alert{antiderivative}: if $g$ and $G$ are functions such that $G' = g$, we say $G$ is an antiderivative of $g$.

}

\frame{
	\frametitle{Activity 4.1.3}
	
	\begin{center}
		\includegraphics[width=3in]{img/4_1_Act2-eps-converted-to.pdf}
	\end{center}

}

\frame{
	\frametitle{Negative Velocity}
	
	The sign of the velocity describes the (1-dimensional) \emph{direction}: forward and backward.

}

\frame{
	\frametitle{Activity 4.1.4}
	
	\begin{center}
		\includegraphics[width=5in]{img/4_1_Act3-eps-converted-to.pdf}
	\end{center}
